...
trait Carnivores extends Animal {
   ...
   */
  override def eat(food: FoodInstance): (Animal, Option[FoodInstance]) = if (food.isInstanceOf[Meat]) super.eat(food) else throw new IllegalArgumentException
}
...
object Carnivores {
  ...
  private class CarnivoresImpl(override val icon: String,
