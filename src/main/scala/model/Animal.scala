...
trait Animal extends Species with Placeable {
  ...
  def canSee(thing: Placeable): Boolean
   ...
   */
  def consume(food: FoodInstance): (Animal, Option[FoodInstance])
...
object Animal {
  ...
  private case class AnimalImpl(override val icon: String, override val name: String, override val size: Size,
    ...
    override def canSee(thing: Placeable): Boolean = this.position.distance(thing.position) <= this.sight
    ...
    override def consume(food: FoodInstance): (Animal, Option[FoodInstance]) = health match {
    ...
    override def quantityFromDeath(): Int = size match {
