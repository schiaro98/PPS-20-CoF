...
class RectangleAreaTest extends AnyFunSuite{
  ...
  test("Create a rectangle area"){
    val rectangle = shape.RectangleArea(Point(0,0), Point(1,1))
    assert(rectangle.width == 1)
    assert(rectangle.height == 1)
  }

  test("Create an illegal rectangle"){
    assertThrows[IllegalArgumentException](shape.RectangleArea(Point(1,1), Point(0,0)))
  }

  test("Find random rectangles"){
    val size = (1000, 1000)
    val r = shape.RectangleArea(Point(0, 0), Point(1, 1))
    for (_ <- 0 to testQ) {
  ...
  test("Check if a Point is inside a rectangle"){
    val r = shape.RectangleArea(Point(0,0), Point(100,100))
